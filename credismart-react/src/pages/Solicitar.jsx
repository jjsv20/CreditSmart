...
const formatoCOP = (valor) => {
  ...
  const handleSubmit = async (e) => {
   ...
   setLoading(true);
    ...
    try {
      ...
      // Reset formulario
      setNombre(""); setCedula(""); setEmail(""); setTelefono(""); setTipo(selectedCredit?.name || "");
      setMonto(""); setPlazo("12"); setDestino(""); setEmpresa(""); setCargo(""); setIngresos(""); setCuota(null);
      setErrors({});
  ...
  {/* Texto azul con el valor en COP (solo si hay monto) */}
  {monto && (
    <div className="mt-1" style={{ color: "#0d6efd", fontWeight: "500" }}>
      {formatoCOP(monto)} COP
    </div>
  )}
</div>
          </div>
